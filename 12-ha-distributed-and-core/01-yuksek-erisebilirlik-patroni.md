...
### c. Alternatif HA Araçları

- **repmgr (Replication Manager):** PostgreSQL için klasik ve yaygın bir replikasyon yönetim aracıdır. Patroni kadar tam otonom (etcd bağımlılığı olmadan) değildir ancak kurulumu ve mimarisi daha basittir. Failover süreçlerini manuel veya `repmgrd` daemon'u ile otomatik yönetebilir.
- **PgPool-II:** Sadece bir Connection Pooler değil, aynı zamanda yük dengeleyici (Load Balancer) ve HA yöneticisidir. Okuma sorgularını (`SELECT`) Standby node'lara dağıtabilir (Read-Write Splitting) ve dahili Watchdog mekanizması ile Sanal IP (VIP) failover yönetimi sağlar.
