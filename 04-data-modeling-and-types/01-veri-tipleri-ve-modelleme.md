# Gelişmiş Veri Tipleri (Arrays, UUID, Range, JSONB)

> **Bölüm Kapsamı:** PostgreSQL'in Zengin Veri Tipleri: Diziler, UUIDv4/v7, Aralık Tipleri, Ağ Adresleri ve Collation
...
| Index verimliliği | Mükemmel | İyi (v7 ile çok daha iyi) |

> [!TIP]
> Modern uygulamalarda B-Tree indeks parçalanmasını (fragmantasyonunu) önlemek için **UUIDv7** (zamana dayalı sıralı UUID) tercih edilmelidir. PostgreSQL 17'de `gen_random_uuid()` standart **UUIDv4** (rastgele) üretmeye devam eder; ancak PG 17 ile birlikte UUID içerisinden zaman bilgisini ayıklayan `uuid_extract_timestamp()` ve sürüm bilgisini veren `uuid_extract_version()` fonksiyonları çekirdeğe eklenmiştir. Doğrudan UUIDv7 üretimi için popüler `pg_uuidv7` eklentisi veya uygulama tarafı kütüphaneleri kullanılabilir.
...
## 11. İkili Veri Saklama Mimarisi: BYTEA vs Large Objects vs S3

Veritabanlarında resim, PDF, video ve sertifika gibi ikili (binary) dosyaların nasıl saklanacağı konusunda 3 temel mimari yaklaşım mevcuttur:

### a. Yöntem 1: `BYTEA` Veri Tipi (Tablo İçinde)

Ham ikili bayt dizileri doğrudan tablonun bir kolonu olarak saklanır.

- **Boyut Sınırı:** Teorik olarak 1 GB (Pratikte satır başına 10–20 MB'ı geçmemelidir).
- **TOAST:** 2 KB'ı aşan veriler otomatik olarak TOAST tablosuna taşınır ve sıkıştırılır.
- **Avantajı:** Kusursuz ACID bütünlüğü. Müşteri silindiğinde vesikalık fotoğrafı da tek bir transaction içinde silinir.
- **Dezavantajı:** Milyonlarca dosya biriktiğinde veritabanı boyutu devasa boyutlara ulaşır (`$PGDATA` şişer); `pg_dump` saatler sürer ve `shared_buffers` gereksiz yere binary sayfalarla kirlenir.

### b. Yöntem 2: Large Objects (`pg_largeobject`, `lo_*` API)

PostgreSQL'in 4 TB'a kadar devasa dosyaları yönetmek için sunduğu özel mekanizmadır. Dosyalar 2 KB'lık parçalar halinde küme genelindeki `pg_largeobject` sistem kataloğunda saklanır. Tabloda ise sadece bu nesnenin kimliği (`OID`) tutulur.

```sql
-- 1. Dosyayı sunucuya Large Object olarak yükle (OID döner)
SELECT lo_import('/tmp/kurumsal_rapor.pdf');
-- Çıktı: 194821

-- 2. Tabloya OID'yi bağla
INSERT INTO documents (doc_name, lo_oid) VALUES ('2024 Yılı Raporu', 194821);

-- 3. Large Object'i dışarı aktar
SELECT lo_export(194821, '/tmp/rapor_kopya.pdf');

-- 4. Silme (DİKKAT: Tablodan satır silinince Large Object otomatik SİLİNMEZ!)
SELECT lo_unlink(194821);
```

> [!WARNING]
> **DBA Large Object Tuzağı:**
> 1. **Yedekleme:** Standart `pg_dump` komutu Large Object'leri **YEDEKLEMEZ**! Mutlaka `pg_dump --blobs` (veya `-b`) bayrağı kullanılmalıdır.
> 2. **Yetim Nesneler (Orphan Blobs):** Tablodan satırı silseniz bile `pg_largeobject` içindeki veri kalır. Bu artık dosyaları temizlemek için periyodik olarak `vacuumlo` aracı çalıştırılmalıdır.

### c. Yöntem 3: Harici Nesne Deposu (S3 / MinIO / Ceph) + Metadata (ÖNERİLEN)

Modern kurumsal mimarilerde en çok tercih edilen bulut standardıdır:
- Dosya S3 veya MinIO bucket'ına yüklenir.
- PostgreSQL tablosunda dosyanın yalnızca **S3 URI'si**, **SHA-256 hash'i**, **MIME tipi** ve **bayt boyutu** saklanır.
- **Kazanım:** Veritabanı hafif kalır, sadece ilişkisel metadata'yı yönetir; dosya trafiği veritabanı motoruna ve RAM'ine yük bindirmeden doğrudan CDN veya S3 üzerinden istemciye akar.

### Karşılaştırma Özeti

| Kriter | BYTEA | Large Objects (`lo`) | Harici Nesne Deposu (S3) |
| :--- | :--- | :--- | :--- |
| **Maksimum Dosya Boyutu** | 1 GB | 4 TB | Sınırsız (Nesne başına 5 TB) |
| **Streaming (Parça Parça) Okuma** | Hayır (Tüm veri RAM'e gelir) | Evet (`lo_read` ile offset) | Evet (HTTP Range Requests) |
| **ACID ve Transaction Bütünlüğü** | Tam | Kısmi | Uygulama mantığına bağlı |
| **Veritabanı Şişmesi (Bloat/Yedek)** | Yüksek risk | Yüksek risk | Sıfır etki (En temiz mimari) |
| **Önerilen Kullanım Alanı** | Küçük ikonlar, SSL anahtarları (<1 MB) | Klasik sistemler, GIS haritaları | PDF, video, fatura, kullanıcı yüklemeleri |
