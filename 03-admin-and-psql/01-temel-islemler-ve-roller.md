# Temel İşlemler ve psql Ustalığı

> **Bölüm Kapsamı:** Komut Satırından Veritabanı Yönetimi ve Yetkilendirme
...
### Kurumsal Bağlantı Profilleri: pg_service.conf

Mikroservislerde veya çok sayıda ortama (Prod, Staging, Test) bağlanan DBA ekiplerinde bağlantı parametrelerini tek tek yazmak yerine **`pg_service.conf`** dosyası kullanılır:

```ini
# ~/.pg_service.conf veya sistem genelinde /etc/postgresql-common/pg_service.conf
...
host=10.0.10.50
port=5432
dbname=ecommerce_stage
user=developer
```

```bash
# Parametre yazmadan servis adıyla anında bağlanın:
psql service=prod_db

# Uygulama bağlantı dizgisi:
# "service=prod_db"
```

### İleri Düzey psql Hileleri ve Üretkenlik

| Meta-Komut / İpucu | Kullanım Amacı ve Pratik Örnek |
| :--- | :--- |
| **`\watch [saniye]`** | Sorguyu döngüsel olarak belirtilen saniyede bir çalıştırıp ekranı yeniler. Canlı kilit ve aktivite izleme için birebirdir:<br>`SELECT pid, state, query FROM pg_stat_activity WHERE state != 'idle' \watch 2` |
| **`\gexec`** | Bir SELECT sorgusunun ürettiği metin satırlarını doğrudan SQL komutu olarak yürütür. Toplu operasyonlar için sihirli bir araçtır:<br>`SELECT 'VACUUM ANALYZE ' \|\| tablename \|\| ';' FROM pg_tables WHERE schemaname = 'public' \gexec` |
| **`\gset [önek]`** | Sorgu sonucunu psql oturum değişkenine atar:<br>`SELECT count(*) AS total FROM users \gset`<br>`\echo Toplam kullanıcı sayısı: :total` |
| **`\copy` vs `COPY`** | **`COPY`:** PostgreSQL sunucusunda çalışır, sunucunun yerel dosya sistemine yazar/okur ve **Superuser** yetkisi gerektirir.<br>**`\copy`:** psql istemcisinin çalıştığı makinedeki dosyayı STDIN/STDOUT akışı üzerinden sunucuya aktarır; superuser yetkisi gerekmez! |

### `~/.psqlrc` ile Terminali Güçlendirme

Kullanıcı ev dizinindeki `.psqlrc` dosyası ile psql terminalinizi renklendirip zenginleştirebilirsiniz:
...
### `search_path` ve Şema Güvenlik Önlemleri

PostgreSQL'de bir tablo adı şemasız yazıldığında (`SELECT * FROM users`), sunucu bu nesneyi `search_path` parametresindeki sıraya göre arar:

```sql
SHOW search_path;
-- Varsayılan: "$user", public
```

> [!WARNING]
> **Güvenlik Riski (Search Path Hijacking):** Kötü niyetli bir kullanıcı `public` şemasına standart sistem fonksiyonlarıyla aynı ada sahip tuzak bir fonksiyon yerleştirebilir.
> - **PostgreSQL 15+ Güvenlik Güncellemesi:** PostgreSQL 15 ile birlikte `public` şemasındaki varsayılan `CREATE` yetkisi tüm kullanıcılardan geri alınmıştır (`REVOKE CREATE ON SCHEMA public FROM PUBLIC`).
> - Güvenlik gerektiren fonksiyonlarda (`SECURITY DEFINER`) mutlaka arama yolunu sabitleyin: `ALTER FUNCTION my_func() SET search_path = pg_catalog, public;`.
