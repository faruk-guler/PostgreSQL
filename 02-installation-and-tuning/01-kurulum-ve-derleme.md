# Kurulum ve OS Optimizasyonu (Production Tuning)
...
> [!IMPORTANT]
> **Platforma Özel Kurulum Rehberleri:**
>
> - **RHEL / Rocky / AlmaLinux:** Bu dağıtımlar için aynı dizindeki 03a numaralı rehberi inceleyebilirsiniz.
> - **Debian / Ubuntu:** Bu dağıtımlar için aynı dizindeki 03b numaralı rehberi inceleyebilirsiniz.
...
### d. NUMA Mimarisi ve Çekirdek Dengeleme Ayarları

Çok soketli fiziksel sunucularda her CPU soketinin kendine ait yerel belleği (NUMA node) bulunur. Bir CPU çekirdeği diğer soketin belleğine eriştiğinde bellek gecikmesi ciddi oranda artar. Ayrıca Linux çekirdeğinin otomatik NUMA dengelemesi veritabanlarında beklenmedik takılmalara (jitter) ve gereksiz swap kullanımına neden olabilir.

```bash
# /etc/sysctl.d/99-postgresql.conf dosyasına ekleyin:

# 1. Otomatik NUMA dengelemeyi kapatın (Veritabanı için zararlıdır)
kernel.numa_balancing = 0

# 2. Bölge geri kazanımını devre dışı bırakın (Uzak node'dan bellek tahsisini yerel disk temizliğine tercih et)
vm.zone_reclaim_mode = 0
```

> [!TIP]
> **PostgreSQL Sürecini Interleave Modda Çalıştırma:**
> Bellek dağılımını tüm NUMA node'larına eşit yaymak için systemd servis tanımında `numactl --interleave=all` kullanılabilir:
> ```bash
> # systemctl edit postgresql-16
> [Service]
> ExecStart=
> ExecStart=/usr/bin/numactl --interleave=all /usr/pgsql-16/bin/postgres -D ${PGDATA}
> ```

### e. Kirli Bellek ve Checkpoint I/O Patlamaları (Dirty Memory Tuning)

Linux'ta varsayılan `vm.dirty_ratio` ve `vm.dirty_background_ratio` yüzdelik olarak çalışır. Yüksek RAM'e (örneğin 256 GB veya 512 GB) sahip sunucularda %10'luk bir kirli bellek bile 25-50 GB kirli verinin RAM'de birikmesi ve Checkpoint anında diske devasa bir I/O dalgasıyla boşaltılarak sistemin donması anlamına gelir.

Bu patlamayı önlemek için yüzdelik yerine bayt bazlı sabit limitler belirleyin:

```bash
# /etc/sysctl.d/99-postgresql.conf

# Arka plandaki flush sürecini erken başlat (örn: 64 MB veya 128 MB)
vm.dirty_background_bytes = 67108864

# Senkron diske yazma sınırını makul bir seviyede tut (örn: 512 MB veya 1 GB)
vm.dirty_bytes = 536870912
```

### f. Disk I/O Zamanlayıcısı (I/O Scheduler) ve Mount Parametreleri

Veritabanı sunucularında disk okuma/yazma kuyruk yönetimi doğrudan sorgu gecikmesini belirler:

- **NVMe / Hızlı SSD Diskler:** Modern PCIe NVMe diskler kendi dahili denetleyicilerine sahiptir; Linux çekirdeğinin kuyruk sıralaması yapması ekstra CPU yüküdür. Scheduler **`none`** olmalıdır.
- **SATA / SAS SSD Diskler:** **`mq-deadline`** tercih edilmelidir.
- **Dönen Manyetik Diskler (HDD):** **`bfq`** veya **`deadline`**.

```bash
# NVMe disk için kontrol et ve uygula:
cat /sys/block/nvme0n1/queue/scheduler
echo none | sudo tee /sys/block/nvme0n1/queue/scheduler
```

**Dosya Sistemi Mount Parametreleri (`/etc/fstab`):**
- `noatime`: Dosyaların her okunduğunda son erişim zamanının (access time) diske yazılmasını engeller. Veritabanı veri dizini (`$PGDATA`) için mutlaka eklenmelidir; aksi takdirde her okuma bir disk yazma işlemine dönüşür!
- `nobarrier`: Yalnızca pil/kondansatör destekli donanımsal RAID önbelleğiniz (BBU / Flash-backed cache) varsa yazma bariyerini devre dışı bırakarak %3–%5 ek yazma hızı kazandırır. Pil koruması yoksa elektrik kesintisinde veri kaybı riski nedeniyle kullanılmamalıdır.

```ini
# /etc/fstab örneği:
UUID=xxxx-xxxx /var/lib/pgsql xfs noatime,nodiratime,logbufs=8,logbsize=256k 0 0
```

### g. CPU Güç Yönetimi ve Scaling Governor

Kurumsal sunucularda işlemciler varsayılan olarak "powersave" veya dinamik güç tasarrufu modunda açılabilir. Bu durum, anlık sorgu patlamalarında CPU frekansının yükselmesinde mikro gecikmelere (frequency ramping latency) yol açar.

```bash
# Tüm çekirdekleri en yüksek performans moduna kilitleyin:
for CPU in /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor; do
    echo performance | sudo tee $CPU
done

# Red Hat / Rocky Linux üzerinde tuned profili ile kalıcı kılma:
...
- **RHEL / Rocky / AlmaLinux:** Detaylar için aynı dizindeki 03a rehberine bakınız.
- **Debian / Ubuntu:** Detaylar için aynı dizindeki 03b rehberine bakınız.
...
## 7. Örnek Veri Seti Kurulumu (Pagila / DVD Rental)

Kitaptaki ilerleyen bölümlerde yer alan kompleks SQL sorgularını, View'ları ve performans testlerini kendi bilgisayarınızda deneyebilmeniz için **Pagila** (PostgreSQL için DVD Rental) örnek veri setini kurmanız tavsiye edilir.
...
2. **Pagila SQL Dosyasını İndirin:**
   ```bash
...
3. **Verileri İçeri Aktarın:**
   ```bash
   psql -d pagila -f pagila-schema.sql
   psql -d pagila -f pagila-insert-data.sql
   ```

Artık `psql -d pagila` komutu ile veritabanına bağlanıp `\dt` yazarak tabloları (film, actor, inventory, vb.) görebilirsiniz.
