# Uygulama Geliştirme Best Practice'leri (App Development)

> **Bölüm Kapsamı:** Python (psycopg 3), Go (pgx), Pipeline Modu, Prepared Statements ve Plan Caching
...
### 5-b. Plan Caching & Invalidation (Önbellek ve Geçersiz Kılma)

PostgreSQL'de bir Prepared Statement çalıştırıldığında iki tip plan üretilir:
1. **Custom Plan (Özel Plan):** Parametrenin gerçek değerine göre optimize edilir.
2. **Generic Plan (Genel Plan):** Parametreden bağımsız, genel bir plandır.

PostgreSQL varsayılan olarak ilk 5 çalıştırmada her defasında **Custom Plan** üretir. 5. çalıştırmadan sonra generic planın ortalama maliyeti, custom planların maliyetinden daha ucuz veya eşitse PostgreSQL **Generic Plan**'a kilitlenir ve tekrar planlama maliyetinden tasarruf eder.

```ini
# Gerekirse session bazında davranış kontrol edilebilir:
# auto (varsayılan) | force_custom | force_generic
SET plan_cache_mode = 'auto';
```

> [!NOTE]
> **Automatic Invalidation:** Eğer tablonun şeması değişirse (`ALTER TABLE`), indeks eklenir/silinirse veya `ANALYZE` sonucu istatistikler köklü biçimde güncellenirse, PostgreSQL önbelleğe aldığı planları otomatik olarak geçersiz kılar (invalidate eder) ve sorguyu yeniden planlar.

### Server-Side Prepare

Eğer aynı sorguyu oturum boyunca defalarca çalıştıracaksanız, doğrudan PostgreSQL tarafında prepare edebilirsiniz:
